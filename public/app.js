...
class SoccerLineupGenerator {
    ...
    /**
     * Markup for one icon from the sprite at /assets/icons.svg.
     *
     * The sprite is generated by scripts/build-icons.js from the Lucide set via
     * Iconify, and is precached by the service worker so icons work offline.
     * Icons inherit the surrounding text colour, so they follow the theme.
     *
    ...
    displayLineup(validationIssues) {
        ...
        const createDropdown = (label, iconId, items) => {
            ...
            iconSpan.innerHTML = this.iconMarkup(iconId);
            const arrowSpan = document.createElement('span');
            arrowSpan.className = 'dropdown-arrow';
            arrowSpan.innerHTML = this.iconMarkup('icon-chevron-down');
        ...
        // Share dropdown
        const shareDropdown = createDropdown('Share', 'icon-share-menu', [copyBtnClone, shareBtnClone]);
        actionButtonsContainer.appendChild(shareDropdown);

        // Export dropdown
        const exportDropdown = createDropdown('Export', 'icon-export-menu', [csvBtnClone, exportBtnClone, printBtnClone]);
